...
```bash
git -C "d:/Projets mylab vs code/be-yours-mylab" checkout -b feat/audit-home-2026-07-08
```

Attendu : `Switched to a new branch 'feat/audit-home-2026-07-08'`.

- [ ] **Step 2 : Lancer le préview local (en arrière-plan, laisser tourner)**

```bash
shopify theme dev --store mylab-shop-3.myshopify.com
```

Attendu : une URL de préview `http://127.0.0.1:9292` s'affiche. Ouvrir la home et vérifier qu'elle rend comme la prod actuelle (hero 3 slides, bandeau défilant, etc.). Ce serveur reste ouvert pour toutes les tâches suivantes.
...
- [ ] **Step 1 : Ajouter la section dans l'objet `sections` de `templates/index.json`**
...
- [ ] **Step 2 : Ajouter la section dans le tableau `order`**
...
**Interfaces:**
- Consumes: la section Liquid existante `sections/featured-collection.liquid` (settings : `collection`, `products_to_show`, `show_view_all`, `heading`, `heading_size`, `heading_alignment`, `heading_tag` — les autres settings prennent leur valeur par défaut du schema). La collection Shopify `best-sellers` (handle vu dans le mega menu du header : `shopify://collections/best-sellers`).
...
- [ ] **Step 2 : Section RDV — corriger le lien du bouton**
...
- [ ] **Step 3 : CTA final — RDV en primaire**
...
- [ ] **Step 1 : Ajouter le setting dans `sections/footer-group.json`**
...
```json
        ...
        "mentions_legales_text": "Mentions légales",
```
...
- [ ] **Step 2 : Créer la page Mentions légales**

Admin > Boutique en ligne > Pages > Ajouter une page. Titre : « Mentions légales » (le handle généré doit être `mentions-legales` — le vérifier dans « Modifier le référencement » et le corriger si Shopify a généré autre chose, sinon le lien footer de Task 5 sera cassé). Contenu : raison sociale, SIREN, adresse du siège, directeur de publication, hébergeur (Shopify Inc.), contact. Visibilité : visible.

Vérification : ouvrir `https://mylab-shop-3.myshopify.com/pages/mentions-legales` → la page se charge, et le lien « Mentions légales » du footer (Task 5) ne renvoie plus 404.
...
- [ ] **Step 4 : Vérifier la collection `best-sellers`**

Admin > Produits > Collections. Confirmer que la collection au handle `best-sellers` existe et contient au moins 4 produits publiés (elle alimente la grille de Task 2). Si elle est vide, y ajouter les formules phares.
...
- [ ] **Step 2 : Pousser sur le thème de développement**
...
- [ ] **Step 3 : Commit final éventuel et récapitulatif**
