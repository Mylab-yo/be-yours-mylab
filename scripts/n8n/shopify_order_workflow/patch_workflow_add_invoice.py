"""Patch the live n8n workflow to insert a 'Create Invoice' Code node between
'Create Sale Order' and 'Activity on Picking'.

Idempotent: re-running detects the node already exists and just updates its jsCode.

Reads the JS body from 02_odoo_client.js + 08_create_invoice.js (concatenated),
...
NEW_NODE_NAME = 'Create Invoice'
ANCHOR_BEFORE = 'Create Sale Order'   # insert AFTER this node
...
def build_js() -> str:
    ...
    invoice = (HERE / '08_create_invoice.js').read_text(encoding='utf-8')
    return odoo_client + '\n\n' + invoice
...
def main():
    ...
    if existing:
        ...
        # Build new node, position halfway between before and after
        ...
        # Rewire connections:
        # before → NEW → after  (instead of before → after)
        if ANCHOR_BEFORE in connections:
            old_main = connections[ANCHOR_BEFORE].get('main', [[]])
            # Replace any link "before → after" with "before → NEW"
            new_main = []
            redirected = False
            ...
            if not redirected:
                # before wasn't directly linked to after — just add a new link to NEW
    ...
    # Build PUT body — n8n REST API has a strict JSON schema; whitelist only
    # supported settings fields and skip None staticData.
