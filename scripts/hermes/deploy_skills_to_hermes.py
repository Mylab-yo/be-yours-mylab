...
SKILLS = ["check-order", "check-customer", "relance-impayes", "faire-of", "gerer-bl", "mylab-rag"]
...
def main():
    ...
    run(ssh, "sleep 7 && docker exec hermes-gateway hermes skills list 2>&1 | grep -iE 'check-order|check-customer|relance-impayes|faire-of|gerer-bl|mylab-rag|installed|name' | head",
        label="verify skills")
