...
def known_parcels(all_parcels):
    ...
    commande splittee : tous les colis du client seraient sinon deverses sur le BL
    restant -> mail en double)."""
    if not all_parcels:
        return set()
...
def build_plan(rows):
    """Retourne (matched, multi, no_partner, no_bl, no_email_rows, already)."""
...
def main():
    ...
    print(f"\n=== NON TRAITES (log) ===")
    if already:
        print(f"  Deja notifies (colis presents sur un autre BL) : {len(already)}")
        for r in already:
    ...
    print(f"\nRESUME: {len(matched)} notifies | {len(multi)} a verifier | "
          f"{len(already)} deja notifies | {len(no_partner)} hors perimetre | "
